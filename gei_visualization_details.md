...
For the purposes of this project, the application is deployed on Render.com (limited bandwidth and memory tier) @ https://geo-equity-index-dashboard.onrender.com/
...
**Vectorized Haversine Distance Calculation**: The most significant performance optimization involves distance filtering for CIMC sites. The application must calculate geodesic distances from the user's search address to all 67,221 hazardous sites in the database using the haversine formula, which accounts for Earth's curvature. Rather than iterating through sites one-by-one with repeated function calls, the implementation employs fully vectorized NumPy operations that process all coordinates simultaneously:

This vectorization strategy reduces distance calculation time from approximately 20 seconds (iterative approach) to under 0.3 seconds—a **~100x performance improvement**. Without this vectorization, the dashboard would exhibit unacceptable latency for users, particularly those searching in densely populated regions with hundreds of nearby hazardous sites.
